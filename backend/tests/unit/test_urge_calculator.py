...
class TestCalculateUrgeStatus:
    ...
    def test_cancelled(self) -> None:
        assert self._call("已取消", date(2026, 6, 1), date(2026, 5, 26)) == "已取消"

    def test_published(self) -> None:
        assert self._call("已发布", date(2026, 6, 1), date(2026, 5, 26)) == "已发布"

    def test_deleted_branch(self) -> None:
        """publish_status 不在 {未发布, 异常} 集合 → '已删除'."""
        assert self._call("已删除", None, date(2026, 5, 26)) == "已删除"
    ...
    def test_within_schedule(self) -> None:
        ...
        assert self._call("未发布", date(2026, 6, 10), date(2026, 5, 26)) == "档期内"

    def test_urge(self) -> None:
        """3 < diff <= 10 → 催发."""
        assert self._call("未发布", date(2026, 6, 4), date(2026, 5, 26)) == "催发"

    def test_important_urge(self) -> None:
        """0 <= diff <= 3 → 重要催发."""
        assert self._call("未发布", date(2026, 5, 27), date(2026, 5, 26)) == "重要催发"

    def test_important_urge_boundary(self) -> None:
        """边界用例：scheduled == today → 重要催发（diff == 0）."""
        assert self._call("未发布", date(2026, 5, 26), date(2026, 5, 26)) == "重要催发"

    def test_overdue(self) -> None:
        """diff < 0 → 超时."""
        assert self._call("未发布", date(2026, 5, 25), date(2026, 5, 26)) == "超时"

    def test_abnormal_uses_same_logic(self) -> None:
        """publish_status='异常' 也走日期分支."""
        assert self._call("异常", date(2026, 5, 27), date(2026, 5, 26)) == "重要催发"
