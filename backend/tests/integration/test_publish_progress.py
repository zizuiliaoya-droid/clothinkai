...
async def _seed_dataset(product_factory, blogger_factory, promotion_factory, factory, tenant):
...
class TestPublishProgress:
    async def test_summary_metrics(
        self, session: Any, tenant_a, product_factory, blogger_factory, promotion_factory, factory
    ) -> None:
        ...
        try:
            ...
            summary = await PublishProgressService(session).get_summary(tenant_a.id, tr)
    ...
    async def test_empty_dataset_null_rates(self, session: Any, tenant_a) -> None:
        tok = tenant_id_ctx.set(tenant_a.id)
        try:
    ...
    async def test_tenant_isolation(
        ...
        tr = resolve_time_range("last_30d")
        summary = await PublishProgressService(session).get_summary(tenant_b.id, tr)
        assert summary.quote_count == 0
