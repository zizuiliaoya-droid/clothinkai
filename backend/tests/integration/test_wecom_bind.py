...
class TestWecomBind:
    async def test_bind_success(
        ...
        try:
            ...
            monkeypatch.setattr(WecomClient, "find_external_userid_by_wechat", _fake_find)
            contact = await WecomBindService(session).bind_contact(blogger.id, tenant_a.id, None)
    ...
    async def test_bind_no_wechat_422(
        ...
        try:
            ...
            with pytest.raises(WecomBloggerNoWechatError):
                await WecomBindService(session).bind_contact(blogger.id, tenant_a.id, None)
        finally:
            tenant_id_ctx.reset(tok)
