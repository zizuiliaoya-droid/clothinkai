...
class AiAdvisoryService:
    ...
    async def _run(self, advice_type: str, payload: dict, messages: list[dict], user: Any) -> dict:
        http = build_ai_http_client()
        try:
            result = await DeepSeekClient(http).chat(messages)
        except AiServiceUnavailableError:
            await self._log(
                advice_type, payload, None, AdviceStatus.DEGRADED.value, None, None, user
            )
    ...
    async def strategy_advice(self, time_range: tuple[date, date], user: Any) -> AiAdviceResponse:
        ...
        report = await ProductionService(self._s).get_report(user.tenant_id, time_range)
        summary = self._summarize_production(report)
        messages = [
            {"role": "system", "content": "你是服装电商运营策略顾问，基于聚合数据给出可执行建议。"},
            {
                "role": "user",
        ...
        result = await self._run(AdviceType.STRATEGY.value, {"summary": summary}, messages, user)
        return self._parse_advice(result["content"], summary)

    # ----------------------------- EP11-S02 异常归因 ----------------------------- #

    async def anomaly_diagnosis(self, alert_id: UUID, user: Any) -> AiAdviceResponse:
    ...
    async def blogger_suggest(
        ...
        candidates = await self._data.candidate_bloggers(user.tenant_id, limit=_MAX_CANDIDATES)
        if not candidates:
            return []
        cand_summary = [
            {
        ...
        await self._run(AdviceType.BLOGGER.value, {"style_id": str(style_id)}, messages, user)
