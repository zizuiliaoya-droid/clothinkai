...
1. **博主库 +26 列**：Blogger 模型加灰豚爬虫字段（3/7/14篇互动、粉丝画像、涨跌指标）+ migration + 灰豚导入映射。
2. **站外推广 +列**：Promotion 模型/响应加 打单地址、发货单号、寄回单号、博主风格、合作形式 等源字段。
3. **店铺数据 24 列**：StoreDailyService 聚合补全千牛 24 列。
4. **投产报表 70 列**：ProductionRow 加中间量列（当前 13 核心派生指标已覆盖决策口径）。

> 这些属后端数据建模扩展（新增列 + migration + 导入/聚合逻辑），是独立于前端的工作量。
